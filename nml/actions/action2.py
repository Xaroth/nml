...
def print_stats():
    ...
    if spritegroup_stats[0] > 0:
        generic.print_info(f"Concurrent spritegroups: {spritegroup_stats[0]}/{total_action2_ids} ({spritegroup_stats[1]})")
    if a2register_stats[0] > 0:
        generic.print_info(f"Concurrent Action2 registers: {a2register_stats[0]}/{total_tmp_locations} ({a2register_stats[1]})")
...
class Action2(base_action.BaseAction):
    ...
    def write_sprite_start(self, file, size):
        assert self.num_refs == 0, f"Action2 reference counting has {self.num_refs:d} dangling references."
        file.comment(f"Name: {self.name}")
...
def make_sprite_group_class(cls_is_spriteset, cls_is_referenced, cls_has_explicit_feature, cls_is_relocatable = False):
    ...
    class ASTSpriteGroup(base_statement.BaseStatement):
        ...
        def __init__(self):
            ...
            """
            raise NotImplementedError(f'__init__ must be implemented in ASTSpriteGroup-subclass {type(self)!r}, initialize(..) should be called instead')
        ...
        def prepare_act2_output(self):
            ...
            if not self._prepared:
                ...
                if len(self._referencing_nodes) == 0:
                    ...
                    generic.print_warning(f"Block '{self.name.value}' is not referenced, ignoring.", self.pos)
        ...
        def collect_references(self):
            ...
            """
            raise NotImplementedError(f'collect_references must be implemented in ASTSpriteGroup-subclass {type(self)!r}')
        ...
        def _add_reference(self, target_ref):
            ...
            if target.is_spriteset():
                ...
                if len(target_ref.param_list) != 0:
                    raise generic.ScriptError(f"Passing parameters to '{target_ref.name.value}' is only possible from a spritelayout.", target_ref.pos)

                self.used_sprite_sets.append(target)
            else:
                if len(target_ref.param_list) != target.num_params:
                    msg = f"'{target_ref.name.value}' expects {target.num_params:d} parameters, encountered {len(target_ref.param_list):d}."
