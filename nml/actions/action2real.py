...
def get_real_action2s(spritegroup, feature):
    ...
    if feature not in action2.features_sprite_group:
        raise generic.ScriptError(f"Sprite groups that combine sprite sets are not supported for feature '{general.feature_name(feature)}'.", spritegroup.pos)
    ...
    if feature in (0x00, 0x01, 0x02, 0x03):
        ...
        msg = f"Sprite groups for feature {feature:02X} will not be supported in the future, as they are no longer needed. Directly refer to sprite sets instead."
    ...
    actions.append(Action2Real(feature, f"{spritegroup.name.value} - feature {feature:02X}", spritegroup.pos, loaded_list, loading_list))
...
def create_spriteset_actions(spritegroup):
    ...
    for feature in spritegroup.feature_set:
        if len(spritegroup.used_sprite_sets) != 0 and feature not in action2.features_sprite_group:
            raise generic.ScriptError(f"Directly referring to sprite sets is not possible for feature {feature:02X}", spritegroup.pos)
        for spriteset in spritegroup.used_sprite_sets:
            ...
            real_action2 = make_simple_real_action2(feature, f"{spriteset.name.value} - feature {feature:02X}", spritegroup.pos, action1.get_action1_index(spriteset))
