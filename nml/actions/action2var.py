...
class Varaction2Parser:
    ...
    def preprocess_storageop(self, expr):
        ...
        if isinstance(expr.register, expression.ConstantNumeric) and expr.register.value > max:
            raise generic.ScriptError(f"Register number must be in range 0..{max:d}, encountered {expr.register.value:d}.", expr.pos)
        if expr.info['perm'] and self.feature not in (0x08, 0x0A, 0x0D):
            raise generic.ScriptError(f"Persistent storage is not supported for feature '{general.feature_name(self.feature)}'", expr.pos)
...
def parse_sg_ref_result(result, action_list, parent_action, var_range):
    ...
    name = f"@return_action_{return_action_id:d}"
...
def parse_result(value, action_list, act6, offset, parent_action, none_result, var_range, repeat_result = 1):
    ...
    if value is None:
        ...
        comment = f"return {value.value:d};"
        result = value
        if not(-16384 <= value.value <= 32767):
            msg = f"Callback results are limited to -16384..16383 (when the result is a signed number) or 0..32767 (unsigned), encountered {value.value:d}."
        ...
        comment = f"return param[{tmp_param:d}];"
        ...
        global return_action_id
        extra_actions, result = create_return_action(value, parent_action.feature, f"@return_action_{return_action_id:d}", var_range)
        return_action_id += 1
        action2.add_ref(result, parent_action)
...
def get_feature(switch_block):
    ...
    if switch_block.var_range == 0x8A:
        ...
        if feature is None:
            raise generic.ScriptError(f"Parent scope for this feature not available, feature: {feature}", switch_block.pos)
