...
def default_60xvar(name, args, pos, info):
    ...
    """
    if len(args) != 1:
        raise generic.ScriptError(f"'{name}'() requires one argument, encountered {len(args):d}", pos)
    return (args[0], [])
...
def tile_offset(name, args, pos, info, min, max):
    if len(args) != 2:
        raise generic.ScriptError(f"'{name}'() requires 2 arguments, encountered {len(args):d}", pos)
    for arg in args:
...
def cargo_accepted_nearby(name, args, pos, info):
    ...
    if len(args) not in (1, 3):
        raise generic.ScriptError(f"{name}() requires 1 or 3 arguments, encountered {len(args):d}", pos)

    if len(args) > 1:
        offsets = args[1:3]
        for i, offs in enumerate(offsets[:]):
            if isinstance(offs, expression.ConstantNumeric):
                generic.check_range(offs.value, -128, 127, f"{name}-parameter {(i + 1):d} '{('x' if i == 0 else 'y')}offset'", pos)
...
def industry_count(name, args, pos, info):
    if len(args) < 1 or len(args) > 2:
        raise generic.ScriptError(f"'{name}'() requires between 1 and 2 argument(s), encountered {len(args):d}", pos)
...
def industry_layout_count(name, args, pos, info):
    if len(args) < 2 or len(args) > 3:
        raise generic.ScriptError(f"'{name}'() requires between 2 and 3 argument(s), encountered {len(args):d}", pos)
